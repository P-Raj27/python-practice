...
decision=True
while(decision==True):
    if(lst[left]==0):        #traverses till it finds 0 at positions and if found one flag is raised
        left=left+1
    else:
        flag_l=1
    if(lst[right]==1):      ##traverses till it finds 1 at positions and if found zero flag is raised
        right=right-1
    else:
        flag_r=1
